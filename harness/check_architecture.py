...
APP_ROOT = ROOT / "backend/src/ops_composer"
...
def _current_package(path: Path) -> tuple[str, ...]:
    ...
    return ("ops_composer", *relative.parts[:-1])
...
def _binding_priority(canonical: str) -> int:
    roots = ("ops_composer", "fastapi", "psycopg")
...
def _is_app_repository_module(module: str) -> bool:
    if not _module_matches(module, "ops_composer"):
        return False
...
def scan(path: Path) -> list[str]:
    ...
    if relative.parts[0] == "domain":
        for module in sorted(candidates):
            forbidden = any(
                ...
                for prefix in ("fastapi", "psycopg", "ops_composer.api", "ops_composer.db", "ops_composer.services", "ops_composer.uow")
    ...
    if is_api(path):
        ...
        for module in sorted(candidates):
            forbidden = any(
                _module_matches(module, prefix)
                for prefix in ("psycopg", "ops_composer.db")
            ) or (
                _module_matches(module, "ops_composer.uow") and not dependency_adapter
            ) or _is_app_repository_module(module)
            if forbidden:
    ...
    if is_service(path):
        for module in sorted(candidates):
            forbidden = any(
                _module_matches(module, prefix)
                for prefix in ("fastapi", "psycopg", "ops_composer.api")
            ) or _is_app_repository_module(module)
            if forbidden:
    ...
    if is_repository(path):
        ...
        for module in sorted(candidates):
            if any(
                _module_matches(module, prefix)
                for prefix in ("fastapi", "ops_composer.api", "ops_composer.services")
            ):
                problems.append(f"{path}: repository cannot import {module}")
            if (
                _module_matches(module, "ops_composer.db.pool")
                or _module_matches(module, "ops_composer.db.repository_connection")
                or _module_matches(module, "ops_composer.db.types")
