...
def _run_checks() -> int:
    ...
    if backend.exists() and require("uv"):
        ...
        if (backend / "src/ops_composer/auth").exists():
            sync.extend(["--extra", "auth"])
        if (backend / "src/ops_composer/events").exists():
        ...
        run(["uv", "run", "--frozen", "--no-sync", "mypy", "src/ops_composer"], backend)
            ...
            compose_environment.write_text(
                "\n".join(
                    (
                        ...
                        "DATABASE_URL=postgresql://ops_composer:harness-only-database-password@db:5432/ops_composer",
                        "APP_ALLOWED_ORIGINS=https://172.20.0.10:8443",
                        "APP_AUTH_RATE_LIMIT_SECRET=harness-only-rate-limit-secret-at-least-32-bytes",
                        "OPS_COMPOSER_MASTER_KEY=MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=",
                        "FORWARDED_ALLOW_IPS=127.0.0.1",
                        "PLAYBOOK_WORKSPACE=./playbooks",
                    )
                )
                + "\n",
                encoding="utf-8",
