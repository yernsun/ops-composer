...
class SqlPredicateBuilder:
    ...
    def add_greater_than_or_equal(self, column: sql.Identifier, name: str, value: object) -> None:
        self._predicates.append(column + sql.SQL(" >= ") + self._bind(name, value))
    ...
    def add_ilike_contains(self, column: sql.Identifier, name: str, value: str) -> None:
        ...
        self._predicates.append(
            column + sql.SQL(" ILIKE ") + self._bind(name, pattern) + sql.SQL(" ESCAPE '\\'")
        )
    ...
    def build(self) -> tuple[sql.Composable, dict[str, object]]:
        predicate = sql.SQL(" AND ").join(self._predicates) if self._predicates else sql.SQL("TRUE")
