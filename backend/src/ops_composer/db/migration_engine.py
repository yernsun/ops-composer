...
class MigrationRunner:
    ...
    async def validate_history(self) -> None:
        ...
        if not await self._tracking_exists():
            raise MigrationError("database is not initialized; run `ops-composer migrate up`")
    ...
    async def validate_current(self) -> None:
        ...
        if pending:
            raise MigrationError(
                f"database has pending migrations: {list(pending)}; run `ops-composer migrate up`"
            )
