...
from ops_composer.api.observability import JsonLogFormatter, current_request_id
from ops_composer.cli import app as cli_app
from ops_composer.db.query import SqlPredicateBuilder, escape_like
from ops_composer.db.repository_connection import PsycopgRepositoryConnection
from ops_composer.db.types import DbConnection, DbPool
from ops_composer.domain.base import StrictDomainModel, to_camel
...
async def test_repository_connection_batches_named_mappings_and_copy_rows() -> None:
    ...
    connection = PsycopgRepositoryConnection(cast(DbConnection, raw_connection), owner)
    query = sql.SQL("INSERT INTO examples (example_id) VALUES (%(example_id)s)")

    assert await connection.execute(query, {"example_id": 1}, prepare=True) == 1
    assert await connection.fetch_one(query, {"example_id": 1}, prepare=False) == {"value": 1}
...
@pytest.mark.asyncio
async def test_http_boundary_adds_request_ids_and_safe_errors() -> None:
    app_transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=app_transport, base_url="http://test") as client:
    ...
    boundary_transport = httpx.ASGITransport(app=boundary)
    async with httpx.AsyncClient(transport=boundary_transport, base_url="http://test") as client:
        plain_response = await client.post("/plain", json={"password": "do-not-log"})
        auth_response = await client.post("/api/v1/auth/login", json={"password": "do-not-log"})
        forbidden_response = await client.get("/forbidden")

    for response in (plain_response, auth_response):
    ...
    assert forbidden_response.json()["code"] == "forbidden"
    assert forbidden_response.json()["message"] == "access denied"


def test_json_log_formatter_emits_structured_safe_fields() -> None:
    record = logging.LogRecord(
        name="ops_composer.test",
        level=logging.INFO,
        pathname=__file__,
        lineno=1,
        msg="completed",
